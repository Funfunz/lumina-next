...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>The name of property shown on the Lumina editor UI</td>
      <td>✅</td>
      <td>undefined</td>
    </tr>
  </tbody>
...
Let's consider two components, `image` and `button`. The `image` component is used to display images and its settings a the `button` component is typically used by the user to interact with the page.
...
The component field refers to the component, in the example `image` or `button`, that is imported from project.
The config field, in the example configImage or configButton, contain the settings of each component.
...
Now let's consider the component `image` and `button` and their configuration.
