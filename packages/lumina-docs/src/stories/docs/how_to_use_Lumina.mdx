...
- `config`: Receives the object `mobileView` that renders a Mobile View in either an `iframe` or a `container`.
- `router`: Sets the URL path for the page/product.
- `getData`:
- `navigate`:
- `component`: The content to be rendered inside the component, check the example [here](/docs/docs-components-required-configuration--documentation).
